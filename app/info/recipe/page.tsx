...
export const metadata: Metadata = {
  title: '생생현장레시피 | 키즈밀',
  description: '키즈밀 영양사가 직접 만든 레시피',
};
    ...
    <>
      ...
      >
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center pt-16">
          ...
          <h1 className="font-serif font-bold text-white text-3xl lg:text-4xl mb-2">생생현장레시피</h1>
          <p className="text-white/70 text-sm sm:text-base">키즈밀 영양사가 직접 만든 레시피</p>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 pb-20 relative z-10">
        {items.length === 0 ? (
          <div className="bg-[#E8F5E9] rounded-2xl p-12 sm:p-16 text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#2D6A4F] flex items-center justify-center text-white font-serif font-bold text-2xl mx-auto mb-5">K</div>
            <p className="font-serif font-semibold text-xl sm:text-2xl text-[#1B4332] mb-2">곧 업데이트될 예정입니다 😊</p>
            <p className="text-[#2D6A4F]/70 text-sm">키즈밀 영양사가 레시피를 열심히 준비하고 있습니다.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item) => (
              ...
              >
                ...
                <div className="p-5">
                  <span className="inline-block text-xs font-semibold px-2.5 py-1 rounded-full bg-[#E8F5E9] text-[#2D6A4F] mb-2">생생현장레시피</span>
