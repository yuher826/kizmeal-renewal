...
interface Props {
  ...
  /** true면 작성 폼을 숨기고 기존 메모 열람만 허용 (CS 쓰기 권한 없는 사용자용) */
  readOnly?: boolean
}
...
export default function InternalNote({ notes, onAdd, readOnly = false }: Props) {
  ...
  return (
    <div className="space-y-3">
      {readOnly ? (
        <p className="text-xs text-gray-400">🔒 메모 작성 권한이 없습니다 (열람만 가능)</p>
      ) : (
